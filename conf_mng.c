...
#define SIP_ENABLED_STATUS "Enabled"
...
{
    if (sip_conf->enabled == NULL || strcmp(sip_conf->enabled, SIP_ENABLED_STATUS))
        return 0;
...
static int conf_write_extensions(sip_conf_t *sip_conf)
{
    ...
    if (sip_conf->enabled != NULL && !strcmp(sip_conf->enabled,SIP_ENABLED_STATUS) && sip_conf->username != NULL &&
